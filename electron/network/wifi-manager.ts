...
/**
 * Parse a nmcli terse-mode line. Fields are separated by `:` but
 * literal colons inside values are escaped as `\:`.
 */
...
export class WiFiManager {
  async scan(): Promise<WiFiNetwork[]> {
    try {
      // Trigger rescan (best-effort, may fail if already scanning)
  ...
  async disconnect(): Promise<boolean> {
    try {
      ...
      console.error('WiFi disconnect failed:', e);
      return false;
    }
  }
