...
- `components/settings/DestinationScreen.tsx` - 目的地管理画面（左: マップ+十字、右: リスト+フォーム）
...
- `components/panels/MapPanel.tsx` - 地図パネル（MapLibre GL JS + PMTiles、GPS 追従、目的地マーカー、ルート直線）
...
- PMTiles ファイル: `~/tiles/`、`tiles/`、`scripts/` を走査して自動検出
- `local-tiles://` Electron カスタムプロトコルで Range リクエスト対応（`protocol.registerSchemesAsPrivileged` + `protocol.handle`）
...
- 左半分: PMTiles マップ + 中央の十字破線（照準）+ 左下に座標表示（`move` イベントで更新）
...
- `map-list-tiles` — PMTiles ファイル一覧（`~/tiles/`、`tiles/`、`scripts/` を走査）→ `{ path, name, size }[]`
