...
- 接続エラー時、main プロセスで USB 電源リセット → 自動再接続（連続リセット回数制限 max 3、接続成功でリセット）
...
- **連続リセット回数制限**: `usbResetCount` / `USB_RESET_MAX = 3` で無限ループ防止。エラー発生 → 即座に USB リセット + 再接続。接続成功（`state === 'connected'`）でカウンターリセット。連続 3 回失敗で停止。`usbResetInProgress` フラグで二重実行防止。初期のクールダウンタイマー方式は「最初のエラーでリセットされず、2回タイムアウトしないとリセットが発動しない」問題があった
